...
import { IntelligenceOverview, CompensationIntelligence, FamilyImpact, SatelliteIntelligence, ParcelIntelligence } from "./IntelligencePages";
import { generateLandParcelReport } from "./pdfGenerator";
...
const nav = [
  ["dashboard", "Command Center"], ["national_dashboard", "National Authority Dashboard"], ["state_dashboard", "State Authority Dashboard"],
  ...
  ["intelligence_overview", "Intelligence Dashboard"], ["compensation_intelligence", "Compensation Intelligence"], ["family_impact", "Family Impact"], ["satellite", "Satellite Intelligence"],
    ...
    <div className="login">
      <div className="login-card">
        ...
        <div style={{ fontSize: "11px", color: "#666", marginTop: "10px" }}>
          Demo Accounts:<br />National: national@landnexus.demo / LandNexus@2026<br />Field Officer: field.officer@landnexus.demo / LandNexus@2026<br />State: state@cbe.ac.in / Tngov@CBE#2026<br />District: district@cbe.ac.in / Tngov@CBE#2026<br />Citizen: citizen@cbe.ac.in / Tngov@CBE#2026
        </div>
      </div>
    </div>
  );
...
function Cards({ d }) {
  let cards = [
    ...
    ["Total Parcels", d.total_parcels], ["Affected Families", d.affected_families], ["Displaced Families", d.displaced_families],
    ...
    ["Legal Disputes", d.legal_disputes], ["SLA Breaches", d.sla_breaches], ["Satellite Changes", d.satellite_changes]
  ];
...
function StateDashboard({ national = false }) {
  ...
  return (
    <>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "15px" }}>
        <h2 style={{ margin: 0 }}>{national ? "National Authority Dashboard" : "State Authority Command Center"}</h2>
...
function Dashboard() {
  ...
  return (
    <>
      <Cards d={d} />
      <Panel title="Parcel-Centric Intelligence"><div className="banner">LAND RECORDS + GIS + SATELLITE + COMPENSATION + FAMILY IMPACT + R&R + RISK + AUDIT</div><p>{d.intelligence_data_note}</p><div className="cards"><div className="metric"><span>R&R Pending</span><b>{d.rr_pending}</b></div><div className="metric"><span>Satellite Evidence</span><b>{d.satellite_evidence_total}</b></div></div></Panel>
      <div className="grid2">
...
function Parcels({ go }) {
  ...
  const testBulkSms = async () => {
    const res = await api("/sms/test-bulk", { method: "POST" });
    setTestSummary(res);
  };
  ...
  return (
    <Panel title="Parcel Register">
      <div className="toolbar" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <ActionButton label="Test SMS — First 6 Parcels" onClick={testBulkSms} />
      </div>
      {testSummary && (
        <div className="notice" style={{ marginBottom: "15px" }}>
          <h4>SMS Test Summary</h4>
          <pre>{JSON.stringify(testSummary.summary, null, 2)}</pre>
          <div style={{ maxHeight: "150px", overflowY: "auto" }}>
            {testSummary.details.map(d => (
              <div key={d.parcel_id}>
                {d.mobile || "No Mobile"} - {d.status}
                {d.error_code ? ` (${d.error_code}: ${d.error_description || "No description"})` : ""}
                {d.message_id ? ` [${d.message_id}]` : ""}
              </div>
            ))}
          </div>
          <button type="button" onClick={() => setTestSummary(null)} style={{ marginTop: "5px" }}>Close Summary</button>
        </div>
      )}
      <Table rows={rows?.items || []} cols={["record_id", "survey_no", "village", "taluk", "district", "area", "classification", "project_id", "acquisition_status", "risk_category"]} onClick={go} actions={r => <button type="button" onClick={e => { e.stopPropagation(); generateLandParcelReport(r); }}>Download Report</button>} />
    </Panel>
  );
}
...
function GIS() {
  ...
  const { data: intelligence } = useData("/intelligence-data/dashboard", 10000);
  const [q, setQ] = useState("");
  ...
  return (
    <Panel title="GIS Parcel Intelligence">
      ...
      {intelligence && <div className="notice" style={{ marginTop: "10px" }}>
        Intelligence overlay: {intelligence.families.affected} affected families · {intelligence.compensation.amount_pending.toLocaleString("en-IN")} pending compensation · {intelligence.satellite.changes} satellite changes.
      </div>}
    </Panel>
  );
}
...
function SLA({ selected }) {
  const { data: operations, error } = useData("/sla/operations", 5000);
  const { data: d } = useData(selected?.project_id ? "/sla/timeline/" + encodeURIComponent(selected.project_id) : null, 5000);
  if (error) return <Panel title="SLA & Timeline"><div className="error">Unable to load SLA data: {error.message}</div></Panel>;
  if (!selected?.project_id) return <Panel title="SLA & Timeline"><p>Open a project to view its timeline, or review current SLA work below.</p><Table rows={[...(operations?.sla_due_soon || []), ...(operations?.sla_breached || [])]} cols={["project_id", "project_name", "current_stage", "due_date", "sla_status", "days_remaining", "days_overdue", "responsible_officer", "reason"]} onClick={row => window.dispatchEvent(new CustomEvent("survi:open-project", { detail: { project_id: row.project_id } }))} /></Panel>;
  return (
...
function AlertTable({ rows, columns, onOpen, onDocument, emptyMessage, actionLabel = "View Details" }) {
      ...
      <table>
        ...
        <tbody>{rows.map((row, index) => (
          <tr key={row.parcel_id || row.project_id || index}>
            ...
            <td><button type="button" onClick={() => onOpen(row)}>{actionLabel}</button>{onDocument && row.parcel_id && <button type="button" onClick={() => onDocument(row)}>View Document</button>}</td>
          </tr>
        ))}</tbody>
      </table>
    </div>
...
function AlertsPage({ go }) {
  ...
  const openDocument = row => { window.dispatchEvent(new CustomEvent("survi:open-documents", { detail: { project_id: row.project_id, parcel_id: row.parcel_id } })); };
  return <Panel title="Operational Alerts">
    ...
    <AlertTable rows={visible} columns={[{key:"type",label:"Alert Type"},{key:"category",label:"Category"},{key:"message",label:"Message"},{key:"project_id",label:"Project"},{key:"survey_no",label:"Parcel / Survey",render:r=>r.parcel_id ? `${r.parcel_id} / ${r.survey_no || "N/A"}` : "N/A"},{key:"district",label:"District"},{key:"created_at",label:"Created"},{key:"severity",label:"Severity"},{key:"read_status",label:"Read Status"},{key:"assigned_to",label:"Relevant User / Role"}]} onOpen={open} onDocument={openDocument} emptyMessage="No alerts found." />
...
function ParcelDetails({ selected }) {
  ...
  return <Panel title={`Parcel Details · ${data.survey_no}`}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "15px" }}>
      <div>
         <button type="button" onClick={() => generateLandParcelReport(data)}>Download Current Report</button>
         <div style={{ display: "inline-block", marginLeft: "10px" }}>
            <ActionButton label="Send SMS Notification" onClick={async () => {
              const res = await api(`/sms/parcel/${data.id}`, { method: "POST" });
              setSmsStatus(res);
              EventBus.dispatch();
            }} />
         </div>
    ...
    {smsStatus && (
      <div className="notice" style={{ marginBottom: "15px" }}>
        {smsStatus.success ? "✓ " : "✗ "}
        {smsStatus.status === "SIMULATED" ? "SMS Simulated (No real SMS was sent)" : smsStatus.success ? "SMS Submitted" : "SMS Failed"}
        <br/>Message ID: {smsStatus.message_id}
        <br/>Status: {smsStatus.status}
        {smsStatus.error_code && <><br/>Reason: {smsStatus.error_code} - {smsStatus.error_description || "No description"}</>}
      </div>
    )}
    <div className="status-card">
      <p><b>Parcel ID:</b> {data.id}</p><p><b>Record ID:</b> {data.record_id}</p><p><b>Project:</b> {data.project_id}</p><p><b>Village / Taluk / District:</b> {data.village} / {data.taluk} / {data.district}</p>
      <p><b>Owner:</b> {data.owner_name || "N/A"}</p><p><b>Mobile:</b> {data.mobile_number || "N/A"}</p><p><b>SMS Status:</b> {data.notification_status}</p>
      <p><b>Risk:</b> {data.risk_category || "N/A"} · Score: {data.risk_score ?? "N/A"}</p><p><b>Acquisition Status:</b> {data.acquisition_status}</p>
    </div>
    <ParcelIntelligence parcelId={data.id} />
  </Panel>;
}
...
function Advanced() {
  ...
  const runActive = async() => setActive(await api('/intelligence/active-learning', {method:'POST',body:JSON.stringify({})}));
...
function App() {
  ...
  useEffect(() => {
    const openProject = event => { setSelected(event.detail); setPage("workflow"); };
    const openDocuments = event => { setSelected(event.detail); setPage("documents"); };
    window.addEventListener("survi:open-project", openProject);
    window.addEventListener("survi:open-documents", openDocuments);
    if (localStorage.getItem("survi_token")) {
      api(AUTH_ME_PATH).then(u => {
        setUser(u);
        if (u.role === 'citizen') setPage('citizen_dash');
        else if (u.role === 'national_authority') setPage('national_dashboard');
    ...
    return () => { window.removeEventListener("survi:open-project", openProject); window.removeEventListener("survi:open-documents", openDocuments); };
  }, []);
  ...
  else if (page === "national_dashboard") content = <StateDashboard national />;
  ...
  else if (page === "intelligence_overview") content = <IntelligenceOverview />;
  else if (page === "compensation_intelligence") content = <CompensationIntelligence user={user} />;
  else if (page === "family_impact") content = <FamilyImpact user={user} />;
  else if (page === "satellite") content = <SatelliteIntelligence user={user} />;

  const allowedNav = nav.filter(n => {
    if (user.role === "citizen") return ["citizen_dash", "grievances"].includes(n[0]);
    if (user.role === "national_authority") return ["national_dashboard", "projects", "gis", "risk", "compensation_intelligence", "family_impact", "satellite", "sla", "alerts", "reports", "audit"].includes(n[0]);
    if (user.role === "state_authority") return ["state_dashboard", "projects", "gis", "risk", "compensation_intelligence", "family_impact", "satellite", "sla", "alerts", "reports"].includes(n[0]);
    if (user.role === "district_authority") return ["district_dashboard", "projects", "parcels", "gis", "risk", "compensation_intelligence", "family_impact", "satellite", "sla", "grievances", "field", "alerts", "reports"].includes(n[0]);
    if (user.role === "admin") return ["dashboard", "users", "projects", "parcels", "documents", "ml", "alerts", "audit", "reports"].includes(n[0]);
    if (["officer", "acquisition_officer"].includes(user.role)) return ["dashboard", "projects", "parcels", "workflow", "risk", "compensation_intelligence", "family_impact", "satellite", "documents", "sla", "alerts", "reports"].includes(n[0]);
    if (user.role === "field_officer") return ["field", "parcels", "gis", "documents", "family_impact", "satellite", "alerts"].includes(n[0]);
    return false;
  });
...
export default App;
export { api, apiBlob, useData, Panel, Table, ActionButton, EventBus, DataPanel, RefreshButton, AlertTable };
