...
import { api, apiBlob, useData, Panel, Table, ActionButton, EventBus, RefreshButton } from "./App";
import { generateOCRPdf } from "./pdfGenerator";
...
export default function DocumentsPage({ user, selected }) {
  ...
  const downloadDocument = async (documentRecord) => {
    try {
      const { blob } = await apiBlob(`/documents/${encodeURIComponent(documentRecord.document_id)}/download`);
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = documentRecord.document_name || `document-${documentRecord.document_id}`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
    } catch (err) {
      const message = err.status === 401 ? "Please log in to download this document." :
        err.status === 403 ? "You do not have permission to download this document." :
        err.message === "DOCUMENT_FILE_NOT_FOUND" ? "The document file is missing from storage." :
      ...
      <Panel title="Document History">
        ...
        <Table rows={documents || []} cols={["document_id", "document_name", "project_id", "parcel_id", "upload_date", "ocr_status", "verification_status"]} actions={(r) => (
          <div style={{ display: "flex", gap: "5px" }}>
            <button onClick={() => downloadDocument(r)}>Download Original</button>
    ...
    <div style={{ background: "#f8f9fa", padding: "15px", borderRadius: "8px", border: "1px solid #ddd" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "10px" }}>
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          <button type="button" onClick={() => generateOCRPdf(documentId, ocrData, fields)}>Download OCR as PDF</button>
