...
def scope_columns(c):
 c.execute("ALTER TABLE users ADD COLUMN scope_state TEXT") if "scope_state" not in {r[1] for r in c.execute("PRAGMA table_info(users)")} else None
 c.execute("ALTER TABLE users ADD COLUMN scope_district TEXT") if "scope_district" not in {r[1] for r in c.execute("PRAGMA table_info(users)")} else None
 c.execute("ALTER TABLE projects ADD COLUMN state_id TEXT") if "state_id" not in {r[1] for r in c.execute("PRAGMA table_info(projects)")} else None
 c.execute("UPDATE projects SET state_id=COALESCE(NULLIF(state_id,''),'Tamil Nadu') WHERE state_id IS NULL OR state_id=''")

def scope_filter(user, project_alias="p"):
 role=user.get("role")
 if role in ("national_authority", "authority", "admin"):
  return "1=1", []
 if role == "state_authority":
  return f"{project_alias}.state_id=?", [user.get("scope_state") or "Tamil Nadu"]
 if role == "district_authority":
  return f"{project_alias}.district=?", [user.get("scope_district") or "Coimbatore"]
 if role in ("acquisition_officer", "officer"):
  return f"{project_alias}.responsible_officer=?", [user["email"]]
 if role == "field_officer":
  return f"EXISTS (SELECT 1 FROM field_assignments fa_scope JOIN parcels pa_scope ON pa_scope.id=fa_scope.parcel_id WHERE pa_scope.project_id={project_alias}.project_id AND fa_scope.officer_email=?)", [user["email"]]
 if role == "citizen":
  return f"EXISTS (SELECT 1 FROM parcels pa_scope WHERE pa_scope.project_id={project_alias}.project_id AND pa_scope.owner_reference=?)", [user["email"]]
 return "0=1", []

def parcel_scope(user, parcel_alias="p"):
 role=user.get("role")
 if role in ("national_authority", "authority", "admin"):
  return "1=1", []
 if role == "state_authority":
  return f"{parcel_alias}.project_id IN (SELECT project_id FROM projects WHERE state_id=?)", [user.get("scope_state") or "Tamil Nadu"]
 if role == "district_authority":
  return f"{parcel_alias}.project_id IN (SELECT project_id FROM projects WHERE district=?)", [user.get("scope_district") or "Coimbatore"]
 if role in ("acquisition_officer", "officer"):
  return f"{parcel_alias}.project_id IN (SELECT project_id FROM projects WHERE responsible_officer=?)", [user["email"]]
 if role == "field_officer":
  return f"EXISTS (SELECT 1 FROM field_assignments fa_scope WHERE fa_scope.parcel_id={parcel_alias}.id AND fa_scope.officer_email=?)", [user["email"]]
 if role == "citizen":
  return f"{parcel_alias}.owner_reference=?", [user["email"]]
 return "0=1", []
def hash_password(p):
...
def audit(user, action, target, details='', **metadata):
 if metadata:
  details = json.dumps({'details': details, **metadata}, default=str, separators=(',', ':'))
...
CREATE TABLE IF NOT EXISTS parcels(id INTEGER PRIMARY KEY AUTOINCREMENT,record_id TEXT UNIQUE,survey_no TEXT,subdivision TEXT,village TEXT,taluk TEXT,district TEXT,owner_name TEXT,mobile_number TEXT,notification_status TEXT DEFAULT 'PENDING',area REAL,classification TEXT,project_id TEXT,latitude REAL,longitude REAL,validation_status TEXT,risk_category TEXT,risk_probability REAL,risk_score REAL,delay_probability REAL,training_label TEXT,created_by TEXT,created_at TEXT DEFAULT CURRENT_TIMESTAMP,remarks TEXT);
...
CREATE TABLE IF NOT EXISTS workflow(project_id TEXT,stage TEXT,status TEXT,notes TEXT,updated_by TEXT,updated_at TEXT DEFAULT CURRENT_TIMESTAMP,PRIMARY KEY(project_id,stage));''')
 ...
 c.execute("UPDATE users SET scope_state='Tamil Nadu' WHERE lower(email) IN ('state@cbe.ac.in','national@landnexus.demo')")
 c.execute("UPDATE users SET scope_district='Coimbatore' WHERE lower(email) IN ('district@cbe.ac.in', 'field@cbe.ac.in', 'field.officer@landnexus.demo')")
 try:
