...
def get_base_metrics(c, query_filter=""):
    ...
    d.update({
        "displaced_families": q("SELECT COALESCE(SUM(displaced),0) FROM family_impacts"),
        "vulnerable_families": q("SELECT COALESCE(SUM(vulnerable),0) FROM family_impacts"),
        "rr_required": q("SELECT COALESCE(SUM(rr_required),0) FROM family_impacts"),
        "rr_completed": q("SELECT COALESCE(SUM(CASE WHEN rr_status IN ('Completed','COMPLETED') THEN 1 ELSE 0 END),0) FROM family_impacts"),
        "satellite_changes": q("SELECT COALESCE(SUM(CASE WHEN change_status != 'NO_SIGNIFICANT_CHANGE' THEN 1 ELSE 0 END),0) FROM satellite_evidence")
    })
    d["rr_pending"] = max(0, d["rr_required"] - d["rr_completed"])
    d["satellite_evidence_total"] = q("SELECT COUNT(*) FROM satellite_evidence")
    d["intelligence_data_note"] = "Intelligence KPIs are calculated from linked application records; satellite imagery is unavailable unless a source reference is provided."
    return d
