...
from backend.core import conn,current_user,audit,UPLOADS,parcel_scope
...
def list_parcels(authorization:str=Header(None),survey_no:str="",subdivision:str="",village:str="",taluk:str="",district:str="",risk_category:str="",project_id:str="",limit:int=100,offset:int=0):
 ...
 scope,scope_args=parcel_scope(u,"p"); q=f"SELECT p.* FROM parcels p WHERE {scope}"; args=list(scope_args)
...
def get_parcel(parcel_id:int,authorization:str=Header(None)):
 ...
 c=conn(); p=c.execute(f"SELECT p.* FROM parcels p WHERE p.id=? AND {scope}",[parcel_id]+scope_args).fetchone()
