...
from backend.core import current_user,conn,audit,scope_filter
...
def list_projects(authorization:str=Header(None),limit:int=100,offset:int=0):
 ...
 scope,args=scope_filter(u,"p")
 c=conn(); rows=[dict(r) for r in c.execute(f"SELECT p.* FROM projects p WHERE {scope} ORDER BY rowid DESC LIMIT ? OFFSET ?",args+[min(limit,500),offset]).fetchall()]; c.close(); return {"items":rows,"count":len(rows)}
@router.get("/{project_id}")
def get_project(project_id:str,authorization:str=Header(None)):
 u=current_user(authorization)
 if not u: raise HTTPException(401,"Authentication required")
 scope,args=scope_filter(u,"p")
 c=conn(); r=c.execute(f"SELECT p.* FROM projects p WHERE p.project_id=? AND {scope}",[project_id]+args).fetchone()
