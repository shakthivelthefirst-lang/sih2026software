...
def operations(authorization: str = Header(None)):
	...
	risk_score = "COALESCE(rp.risk_score, pa.risk_score, 0)"
	...
	result = {
		...
		"risk": {
			...
			"average_score": c.execute("SELECT ROUND(AVG(COALESCE(rp.risk_score, pa.risk_score)),2) FROM parcels pa LEFT JOIN risk_predictions rp ON rp.id=(SELECT MAX(id) FROM risk_predictions WHERE parcel_id=pa.id) WHERE COALESCE(rp.risk_score, pa.risk_score) IS NOT NULL").fetchone()[0] or 0,
