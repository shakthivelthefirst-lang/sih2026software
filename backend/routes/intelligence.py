...
def active_learning(payload:dict, authorization:str=Header(None)):
    user_or_401(authorization)
    if not payload.get('parcels'):
        c=conn()
        payload['parcels']=[dict(row) for row in c.execute("""
            SELECT id AS parcel_id, COALESCE(risk_score,0) AS risk,
                   COALESCE(risk_probability * 100,0) AS uncertainty,
                   (documentation_pending + compensation_pending + rehabilitation_pending) AS missing_evidence,
                   affected_families AS connected_parcels
            FROM parcels
            WHERE risk_category IN ('HIGH','CRITICAL')
            ORDER BY COALESCE(risk_score,0) DESC, id LIMIT 50
        """).fetchall()]
        c.close()
    rows=[]
